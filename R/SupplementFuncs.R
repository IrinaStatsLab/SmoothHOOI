...
# upper - maximum number of rows to be masked
.mask4 <- function(tnsr, lower, upper){
  a <- tnsr@modes[1]
  n <- tnsr@modes[3]
  for (i in 1:n){
...
# mean_G: mean vector (e.g., mu11, mu12, mu21, mu22, mu31, mu32) for G
...
# percent: percent of data to be masked, if pattern = "random"
# lower, upper: range of number of timepoints to be masked, if pattern = "structured"
sim_data2 <- function(L, R, mean_G, cov_G, E, p, noise_level, pattern, percent, lower, upper){
  r1 <- ncol(L)
  r2 <- ncol(R)
